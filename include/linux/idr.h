...
/*
 * 5.x's ida_alloc_range()/ida_free() expressed over this tree's 4.14 IDA API.
 * ida_simple_get() takes an exclusive @end, where 0 means "no upper bound".
 */
static inline int ida_alloc_range(struct ida *ida, unsigned int min,
				  unsigned int max, gfp_t gfp)
{
	return ida_simple_get(ida, min, max == ~0U ? 0 : max + 1, gfp);
}
